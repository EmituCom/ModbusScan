...
console.log(`Using ${REGISTERS_CONFIG.length} registers for monitoring.`);
...
function connectModbus() {
    if (client.isOpen || isModbusConnected) {
        console.log('Connection established or in progress.');
        return;
    }
    ...
    //console.log(`Trying to conect by RTU on ${MODBUS_SERIAL_PORT}...`);
    ...
    client.connectRTUBuffered(MODBUS_SERIAL_PORT, MODBUS_SERIAL_OPTIONS)
        .then(() => {
            ...
            updateStatus(`Connected to gate ${MODBUS_SERIAL_PORT}. Start reading...`, -1);
            startReading();
        })
        .catch((err) => {
            isModbusConnected = false;
            overallStatus = `{red-fg}Connection Failed{/}`;
            updateStatus(`Fail on conecting: ${err.message.substring(0, 30)}. Trying...`, -1);
            //console.error('Error in RTU connection:', err);
...
function updateStatus(currentAction = overallStatus, progress = currentProgress) {
    ...
    if (isModbusConnected && client.isOpen) {
        connectionStr = `{green-fg}ONLINE{/} (${MODBUS_SERIAL_PORT})`;
...
// Função para fazer a leitura inicial e contínua
...
async function readAndUpdateTable() {
    if (!isModbusConnected || !client.isOpen || isInputActive) {
        updateStatus(`waiting for conection...`, -1);
        return;
    }
        ...
        try {
            ...
            switch (reg.type) {
                ...
                case "DiscreteInput":
                    ...
                    break;
                default:
                    throw new Error(`Type nor supported: ${reg.type}`);
            }
            ...
            if (err.modbusCode) {
                statusStr = `{red-fg}Err ${err.modbusCode}{/}`;
